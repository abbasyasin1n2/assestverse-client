...
const UpdateAsset = () => {
  ...
  useEffect(() => {
    if (asset) {
      ...
      setValue("type", asset.type || "returnable");
      setValue("category", asset.category || "");
      setValue("quantity", asset.quantity || 1);
      setValue("description", asset.description || "");
      setValue("purchaseDate", asset.purchaseDate?.split("T")[0] || "");
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Send to server
      const response = await axiosInstance.patch(`/assets/${id}`, updateData);
  ...
  if (isError) {
    return (
      <div className="alert alert-error max-w-xl mx-auto">
        <span>Failed to load asset. It may have been deleted.</span>
        <button className="btn btn-sm" onClick={() => navigate("/dashboard/asset-list")}>
          Go Back
        </button>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="p-3 bg-secondary/10 rounded-xl">
          <FiEdit2 className="h-8 w-8 text-secondary" />
        </div>
        <div>
          <h1 className="text-2xl font-bold">Update Asset</h1>
          <p className="text-base-content/70">Edit asset information</p>
        </div>
      </div>
